...
numeric_columns <- colnames(data)[!(colnames(data) %in% c("File Name", "Player", "Velocity"))]
...
for (measure in numeric_columns) {
  ...
  result <- rmcorr(Player, Velocity, measure, dataset = data)
...
write.csv(coefficients_df, file = "Velocity RMCorr (main data).csv", row.names = FALSE)
