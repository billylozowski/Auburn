...
numeric_columns <- colnames(data)[!(colnames(data) %in% c("File Name", "Player", "Displacement (m)"))]
...
for (measure in numeric_columns) {
  data[[measure]] <- as.numeric(as.character(data[[measure]]))
...
for (measure in numeric_columns) {
  ...
  result <- rmcorr(Player, `Displacement (m)`, measure, dataset = data)
...
write.csv(coefficients_df, file = "Displacement RMCorr (main data).csv", row.names = FALSE)
