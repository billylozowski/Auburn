```{r}
# DEFINE PARTICIPANT
participant_code <- "BB_C_WW_01_07-12-22"
```
...
```{r}
# CREATE A DATA FRAME WITH ONLY THE PARTICIPANT'S DATA IN IT
...
```{r}
# DETERMINE THE PEER GROUP BSED ON PARTICIPANT'S AGE
...
```{r}
# SEPARATE ROM INTO A SEPARATE DATA FRAME AND PLOT
...
plot <- ggplot(peer_ROM, aes(x = value, y = name, fill = name)) +
  ...
  theme(panel.grid.major = element_blank(), # remove grid lines
        ...
        plot.title = element_text(face = "bold", size = 16, hjust = 0.5),  # format plot title
...
rm(participant_ROM, peer_ROM, ROM_plot, ROM_colours) 
```

```{r}
# SEPARATE TROM INTO A SEPARATE DATA FRAME AND PLOT
...
rm(participant_TROM, peer_TROM, TROM_plot, TROM_colours)
```

```{r}
# SEPARATE STRENGTH INTO A SEPARATE DATA FRAME AND PLOT
...
plot <- ggplot(peer_Strength, aes(x = value, y = name, fill = name)) +
  ...
  xlab("Newton-metres (Nm)") + 
  ggtitle("Isometric Strength") +
  theme_minimal() +
  theme(panel.grid.major = element_blank(), # remove grid lines
        panel.grid.minor = element_blank(), # remove grid lines
        plot.background = element_rect(fill = "white"),
...
Strength_plot <- plot + geom_point(data = participant_Strength, aes(x = value, y = name), color = "#EF6C00", size = 3)
print(Strength_plot)
...
ggsave(filename = file_name, plot = Strength_plot, device = "png", width = 8, height = 6, units = "in", dpi = 300)

rm(participant_Strength, peer_Strength, Strength_plot, Strength_colours, plot)
```
...
rm(col_index, variable_range, summary_tables, summary_data, 
   col_name, colnames_data, new_colnames, file_name)
```
