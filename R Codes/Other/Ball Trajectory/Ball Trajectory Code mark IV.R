...
calculate_trajectory <- function(velocity, release_angle, horizontal_angle, release_height, release_distance, num_steps = 100) {
  ...
  time_intervals <- numeric(num_steps)
  
  trajectories <- vector("list", length = length(velocity))
  
  for (i in seq_along(velocity)) {
    time_of_flight <- (2 * velocity[i] * sin(release_angle[i] * pi / 180)) / g
    time_intervals <- seq(0, time_of_flight, length.out = num_steps)
    
    x <- release_side[i] * cos(horizontal_angle[i] * pi / 180) * release_height[i]
    y <- velocity[i] * sin(release_angle[i] * pi / 180) * release_height[i] - 0.5 * g * time_intervals^2
    z <- release_angle[i] * g * release_height[i]
    
    trajectory <- data.frame(Time = time_intervals,
                             X = x,
                             Y = y,
                             Z = z,
                             Pitch_ID = rep(i, num_steps))
