...
# Read the file and name columns
  ...
  data <- read.csv(file_path, header = TRUE, skip = 3, check.names = FALSE) 
  
# Filter data by Pitch Type and select only relevant columns
  ball_data <- data %>%
    filter(`Pitch Type` == "Fastball") %>% # only when data is pitching
    select(No, `Pitch Type`, Velocity, `Release Angle`, `Horizontal Angle`, `Release Height`, `Release Side`, `Release Extension (ft)`)
  
  ball_data <- ball_data %>%
    mutate_at(vars(-matches("Pitch Type")), as.numeric)
  
# Remove rows with all NA values
  ball_data <- ball_data[complete.cases(ball_data), ]
  rm(file_path)

# # Rapsodo (hitting unit for throwing)
# library(tidyverse)
# ball_data <- data %>%
#   select(No, HitID, `ExitVelocity (mph)`, StrikeZoneX, StrikeZoneY, LaunchAngle, ExitDirection)
...
# Throw length (start point to target)  
  throw_length <- 18.4404 # (60.5 ft in metres)
...
# Release Parameters
  horizontal_angle <- ball_data$`Horizontal Angle`
  vertical_angle <- ball_data$`Release Angle`
  distance <- throw_length  - (ball_data$`Release Extension (ft)` * ft_to_m) # throw_length - extension (if else loop)
  height <- ball_data$`Release Height` * ft_to_m
  velocity <- ball_data$Velocity * mph_to_mps
  side <- ball_data$`Release Side` * ft_to_m

# Merge release parameters into a data frame
  converted_data <- data.frame(velocity, vertical_angle, horizontal_angle, distance, side, height)
  rm(ball_data, throw_length, ft_to_m, mph_to_mps, height, vertical_angle, horizontal_angle, velocity, side)
...
# Convert angle to radians
  theta <- as.numeric(vertical_angle) * pi / 180
...
# Remove the row with the name "results"
  ...
  rm(converted_data, distance, g, height, i, result, variable_O, velocity, vertical_angle, trig_data)
...
# Set aim point
  in_to_m <- 0.0254
  aim_height <- 21 * in_to_m # change this value as required
  aim_side <- 0 * in_to_m# change this value as required (only when pitching to off-centre quadrants)
  aim_point <- data.frame(aim_height, aim_side)
  rm(in_to_m, aim_height, aim_side)
...
no_bounce <- data.frame(no_bounce, displacement)
...
# Create a matrix from the 'target_side' and 'target_height' variables
  data_matrix <- cbind(no_bounce$target_side, no_bounce$target_height)
  data_matrix <- as.numeric(data_matrix)

  # Perform principal component analysis
  pca <- prcomp(data_matrix, scale = TRUE)
  
  # Check if any principal components were computed
  if (length(pca$sdev) == 0) {
    cat("Error: No principal components were computed\n")
  } else {
    ...
    num_components <- length(pca$sdev)
    
    if (num_components < 2) {
      cat("Error: Insufficient principal components for analysis\n")
    } else {
...
labs(x = "Lateral Displacement (m)", y = "Vertical Displacement (m)", color = "Displacement")

rm(aim_point, eigenvectors, results, data_matrix, displacement, eigenvalues, num_components, pca)
...
no_bounce$AIM_score <- ((no_bounce$velocity * no_bounce$distance) / no_bounce$displacement) / 100 # /100 is a scaling factor!


# RENAME COLUMN HEADERS
library(dplyr)
colnames(no_bounce) <- c("Velocity (m/s)", "Release Angle", "Throw Distance", 
                                   "Vertical Location (m)", "Lateral Location (m)", "Height From Target (m)", "Side From Target",
                                   "Displacement (m)", "A.I.M. Score")

# Rename Output
Final_Output <- no_bounce
